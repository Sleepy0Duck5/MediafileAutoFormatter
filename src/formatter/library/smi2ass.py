...
# Forked from: https://github.com/luhavis/smi-to-ass (2025.07.21). License of this code inherits the original repo's license.

import chardet, sys, re
...
Style: Default,"""
    + default_font_name
    + """,66,&H00FFFFFF,&H0300FFFF,&H00000000,&H02000000,-1,0,0,0,100,100,0,0,1,3,0,2,65,65,65,1
...
def smi2ass(smi_sgml):
    ...
    smi_sgml = re.sub(
        r"< *[Ss][Yy][Nn][Cc] +", "</sync><sync ", smi_sgml
    ...
    for lang_idx, lang in enumerate(mln):
        ...
        if len(ass_lines) > 0:
            asscontents = (script_info + styles + events + "".join(ass_lines)).encode(
                "utf-8"
            )
    ...
    for line_idx, item in enumerate(sln):
        ...
        if line_idx + 1 < len(sln) and not li == None and not li1 == None:
                ...
                if not col == None:
                    ...
                    if hexcolor_regex is not None:
                        ...
                        try:
                            ...
                            print(
                                "Failed to convert a color name: %s"
                                % color["color"].lower()
                            )
            ...
            if minor >= 7:
                ...
                raise Exception("Does not supports under python 3.7")
...
def separate_by_lang(smi_lines):
    ...
    for index, lang in enumerate(langcount):
        portion = float(len(multiLanguageDict[lang[0]])) / float(
            langcount[len(langcount) - 1][1]
        )
        langcount[index].insert(
            2,
            float(len(multiLanguageDict[lang[0]]))
            / float(langcount[len(langcount) - 1][1]),
        )
        try:
    ...
    # covert to real language name and merge to largest
        ...
        for index, langc in enumerate(langcount):
            if (
                langc[5] == 1 and langc[4] == 1
            ):  # less than 10% and coverted to real lang name
